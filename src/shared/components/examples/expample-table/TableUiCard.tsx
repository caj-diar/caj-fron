...
export const TableCardUi: React.FC = () => {
  ...
  const filteredItems = React.useMemo( () => {
    ...
    if ( statusFilter !== "all" && Array.from( statusFilter as Set<string> ).length !== statusOptions.length ) {
      filteredUsers = filteredUsers.filter( ( user ) =>
  ...
  const renderCell = React.useCallback( ( user: UserData, columnKey: string ) => {
            ...
            <Dropdown>
              <DropdownTrigger>
                <Button isIconOnly size="sm" variant="light">
                  <IoArrowBack className="text-default-300" />
                </Button>
              </DropdownTrigger>
              <DropdownMenu>
      ...
      <div className="flex flex-col gap-4">
        <div className="flex justify-between gap-3 items-end">
          ...
          <div className="flex gap-3">
            ...
            <Button color="primary" endContent={ <IoAddOutline size={ 20 } /> }>
              Add New
            </Button>
          </div>
        </div>
