...
import { IClient, useAccounts } from '../../clients';
import { ILoanByClient } from '../../loans/interfaces/ILoansByClient';
import { useRefinanceAddLoan } from '../../loans/hooks/useRefinanceLoan';
import { LoanInputs, loanSchema } from '../../loans/validators';
...
export const RefinanceLoan = ( { loan, client }: Props ) => {
  const { isOpen, onOpen, onOpenChange, onClose } = useDisclosure();
  const { addLoan } = useRefinanceAddLoan( loan.id );
  ...
  useEffect( () => {
    ...
    const paymentFrequency = undefined;
    const status = loan.status as "ACTIVO" | "TERMINADO" | "ATRASADO" | undefined;
    const dueDays = undefined;

    // Calcular el total pagado incluyendo pagos parciales
    const totalPaidAmount = loan.schedules.reduce( ( total, schedule ) => {
      return total + ( schedule.paidAmount || 0 );
    }, 0 );

    // Calcular el monto restante
    const remainingAmount = loan.totalAmountWithInterest - totalPaidAmount;
